...
export function setupAdmin({ api, getBaseURL, cardHTML, avatar, escape: esc, showSettings, toast }) {
  ...
  // Same Wall setup dialog as the footer: hall name, omg.dev backend, Google
  // Form link and Apps Script feed URL, reachable without leaving the board.
  $('#admin-settings').addEventListener('click', showSettings);
  ...
  function review() {
    const p = active;
    const field = (name, title, value, max, type = 'text') => `<label>${title}<input type="${type}" name="${name}" value="${esc(value)}" maxlength="${max}" required></label>`;
