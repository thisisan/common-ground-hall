...
- Residents add up to 5 social accounts from any supported platform (Instagram, TikTok, X, Threads, Snapchat, Telegram, Discord, WeChat, XHS, LinkedIn, Facebook, YouTube, Website), stored as a `socials` list of `{ platform, value }`. Handle platforms accept a username; link platforms require an HTTPS URL on that platform's domain. Duplicate platforms are rejected. Domain, length and protocol checks all run on the server, and `handle`/`xhs`/`linkedin` are still mirrored for older feeds.
